...
def fetch_articles_concurrently(article_list, fetch_func):
    results = []
    progress_bar = progress_placeholder.progress(0.0, text="부문 수집 중...")
...
def parse_yonhap():
    collected, page = [], 1
    status_placeholder.info("🔍 [연한뉴스] 목록 수집 중...")
    while True:
        ...
        for item in items:
            ...
            if start_dt <= dt <= end_dt:
                collected.append({"source": "연한뉴스", "datetime": dt, "title": title_tag.text.strip(), "url": f"https://www.yna.co.kr/view/{cid}"})
        page += 1
    return fetch_articles_concurrently(collected, get_yonhap_content)
...
if st.button("📅 기사 수집 시작"):
    ...
    status_placeholder.success(f"✅ 총 {len(articles)}개의 기사를 수집했습니다.")
